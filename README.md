...
本项目不是简单调用大模型返回代码，而是把一次代码生成任务拆成可追踪、可测试、可复盘的工程流程。
...
没有 API Key 也能运行，系统会使用离线兜底算法模板完成本地任务处理。页面输入框填写的 API Key 只通过当前请求头传给后端，不会写入数据库、日志或报告。
...
| 前端生产构建 | 通过 |
| 生产依赖 audit | 0 漏洞 |
| 30 题回归 Benchmark | Offline_Fallback 100.0%，Online_Bailian 100.0% |
...
当前 Benchmark 是项目内置的回归评测集，主要用于验证任务链路、代码执行器、评测器、报告生成和常见算法题型覆盖情况。本地最近一次跑批结果为 Offline_Fallback 100.0%、Online_Bailian 100.0%。

这个结果不等同于未知题目的泛化能力。离线兜底属于规则和模板基线，适合保证系统在无 API Key 或接口失败时仍有稳定输出；如果要评估模型在新题上的效果，应另外准备未见题集，并使用有效百炼 API Key 重新跑批对比。
...
- 在没有 API Key 或接口失败时，仍能用离线兜底模板完成稳定输出。

## 安全说明

本项目执行器包含 AST 检查、临时目录、超时、输出截断、危险调用拦截等保护，适合受控算法代码执行。若要运行不可信用户代码，建议启用 Docker Sandbox 或 E2B，并继续限制网络、CPU、内存、进程数和文件系统权限。

## 旧版 Streamlit 单文件运行

仓库仍保留单文件版本，便于快速运行或对比：
